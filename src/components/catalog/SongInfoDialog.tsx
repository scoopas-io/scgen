import { Music, Hash, Clock, Gauge, Key, FileText } from "lucide-react";
...
export function SongInfoDialog({ song, albumName, artistName, open, onOpenChange }: SongInfoDialogProps) {
  ...
  // Viewer: only title card + technical data
  if (!isAdmin) {
    return (
  ...
  const { User, DollarSign, Calendar, Shield, FileCheck, Building2 } = require("lucide-react");
